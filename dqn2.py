...
class QLearner(object):
    ...
    def __init__(self,
        ...
        q_t_taken = tf.reduce_sum(tf.multiply(self.q_t, tf.one_hot(self.act_t_ph, self.num_actions)), axis = 1)
    ...
    def step_env(self):
        ...
        if not self.model_initialized:
            ...
            # exploitation
            imgs_in = self.replay_buffer.encode_recent_observation()
            q = self.session.run(self.q_t,
                            feed_dict={ self.obs_t_ph: imgs_in[None, :] })
            action = np.argmax(q)
